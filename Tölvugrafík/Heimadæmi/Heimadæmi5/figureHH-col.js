...
var legAnimationSpeed = 20;  // Speed of leg movement
var legMaxAngle = 45;       // Maximum angle for leg swing
var legDirection = 1;       // 1 for forward, -1 for backward (initial direction)
...
var theta = [40, -10, 140, 10, -140, 40, 45, -10, -45, -40, 0];
...
var render = function() {
    ...
    theta[leftUpperLegId] += legDirection * legAnimationSpeed * 0.1;
    theta[rightUpperLegId] -= legDirection * legAnimationSpeed * 0.1;
    theta[leftLowerLegId] += legDirection * legAnimationSpeed/2 * 0.1;
    theta[rightLowerLegId] -= legDirection * legAnimationSpeed/2 * 0.1;

    theta[leftUpperArmId] -= legDirection * legAnimationSpeed * 0.1;
    theta[rightUpperArmId] += legDirection * legAnimationSpeed * 0.1;
    theta[leftLowerArmId] -= legDirection * legAnimationSpeed/2 * 0.1;
    theta[rightLowerArmId] += legDirection * legAnimationSpeed/2 * 0.1;

    // Reverse direction when reaching max angles
    if (theta[leftUpperLegId] > legMaxAngle || theta[leftUpperLegId] < -legMaxAngle) {
        legDirection *= -1;
    }

    // Initialize nodes after changing theta values
    for(i=2; i<9; i++) {
        initNodes(i);
    }
